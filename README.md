...
已为所有 Python 并发模式创建了 bend-lang/ 目录，每个模式都用 Bend 的函数式/并行范式实现了对应映射。所有 9 个程序都通过了 bend
  check 语法验证，且运行结果均与预期输出一致。
  ...
  │                模式                │                对应 Python 源                 │               方法映射                │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ actor/print_server.bend            │ python/actor/print_server.py                  │ 通过尾递归循环传递状态的 ADT 消息分发 │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ csp/channel_demo.bend              │ python/csp/channel_demo.py                    │ 将 Channel 实现为惰性列表——生产者构建 │
  │                                    │                                               │ 流，消费者归约                        │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ shared_memory/bank_account.bend    │ python/shared_memory_lock/bank_account.py     │ 纯函数式的状态传递，无锁地保证资金守  │
  │                                    │                                               │ 恒                                    │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ shared_memory/producer_consumer.be │ python/shared_memory_lock/thread_producer_con │ 通过列表串联与交织实现函数式生产者-消 │
  │ nd                                 │ sumer.py                                      │ 费者                                  │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ continuation_cps/continuation_cps. │ python/continuation_cps/continuation_cps.py   │ CPS 变换——Bend                        │
  │ bend                               │                                               │ 有天然的一等闭包，且无需分离蹦床      │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ stackless_coroutine/yield_fibonacc │ python/stackless_coroutine/yield_fibonacci.py │ 惰性斐波那契数列——Bend                │
  │ i.bend                             │                                               │ 的按需规约天然等价于惰性求值          │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ stackful_coroutine/deep_recursion. │ python/stackful_coroutine/greenlet_demo.py 和 │ 深度递归与协程风格的交织——HVM2        │
  │ bend                               │  stackless_vs_stackful.py                     │ 不存在调用栈溢出                      │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ stm/stm_model.bend                 │ python/stm/stm_model.py                       │ 无冲突的事务性状态：纯函数式中不存在  │
  │                                    │                                               │ 竞争条件                              │
  ├────────────────────────────────────┼───────────────────────────────────────────────┼───────────────────────────────────────┤
  │ t9_combinations.bend               │ python/t9_combinations.py                     │ 递归、回溯与 CPS 三种方式实现 T9      │
  │                                    │                                               │ 组合（生成器为函数式风格）            │
  └────────────────────────────────────┴───────────────────────────────────────────────┴───────────────────────────────────────┘

  核心思路

  Bend 是纯函数式语言，没有可变状态、线程、锁或显式并行构造。与 Python 的映射要点是：

  - Actor / CSP / 锁 → 通过尾递归显式传递的函数式状态
  - 生产-消费 → 列表构造（生产者）与归约（消费者），由 HVM2 自动并行
  - 协程 → 递归函数织入——Bend 中没有调用栈，所有调用都是“无栈的”
  - STM → 纯事务：输入状态 → 输出状态、无条件冲突、守恒性由代数性质保障
...
## bend-lang on GPU/CUDA by 
可以在编译器自动实现并行化解释/编译吗？ - 酱紫君的回答 - 知乎
https://www.zhihu.com/question/646234682/answer/3517299156
这个逆天的理论被称为 交互式组合子 (IC, Interaction Combinators[1]), 该理论可以追溯到 1997 年的一篇论文.
